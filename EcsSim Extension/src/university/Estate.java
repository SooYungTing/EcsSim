...
public class Estate {
  // list of all the facilities (e.g halls, labs, theatres,library)
  ...
  - types supported are "Hall", "Lab", "Library" and "Theatre"
  ...
  - it is calculated based on the minimum capacity among halls, labs, library and theatres*/

  public int getNumberOfStudents() {
    int totalHallCapacity = 0;
    int totalLabCapacity = 0;
    int totalTheatreCapacity = 0;
    int totalLibraryCapacity = 0;
    ...
    for (Facility facility : facilities) {
      ...
      } else if (facility instanceof Library) {
        totalLibraryCapacity += ((Library) facility).getCapacity();
        }
    }
    ...
    // returns the minimum of the total capacities
      return Math.min(Math.min(totalHallCapacity, totalLabCapacity), Math.min(totalTheatreCapacity, totalLibraryCapacity));
